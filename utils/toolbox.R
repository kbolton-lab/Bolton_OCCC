...
plot_forest <- function(class_results, x = "class", y = "estimate", ymin = "conf.low", ymax = "conf.high",
  ...
  output_plot <- ggplot(class_results, aes_string(x = x, y = y, ymin = ymin, ymax = ymax, col = col, fill = fill)) + 
    ...
    geom_text(aes_string(label = label, vjust = -0.7), position = position_dodge(width = dodge_width), color = ggthemr::swatch()[1], size = or_s, alpha = .9) +
    geom_hline(yintercept = yinter, color = "black", linetype = "solid") +
    coord_flip() +
    ggtitle(title)
...
summar = function(model, truncate_p = TRUE, p_het = NULL, CI = TRUE, signif = FALSE, n = FALSE, latex = TRUE, ref = TRUE, sep = F) {
    ...
    if ('geese' %in% class(model)) {
      ...
      summary = as.data.frame.matrix(
        ...
        summary = sjPlot::get_model_data(model) %>%
    ...
    if (latex) {
      summary = summary %>%
        rename(`95\\% CI` = `CI`) %>%
        mutate(pval = kableExtra::cell_spec(pval, "latex", bold = ifelse(pval < 0.05, T, F)))
    }

    if (sep) {
      summary = summary %>% tidyr::separate(CI, c('upper', 'lower'), '-') %>%
...
ord_summary = function(model) {
    ...
    as.data.frame(
        ...
        as.data.frame(exp(confint.default(model))) %>% rename(conf.low = `2.5 %`, conf.high = `97.5 %`), 
...
format_variable = function(vars) {
  as.character(vars) %>%
    str_remove_all(regex('binary|scaled|bin|verbose|_b$|^g_|^ind_|^pct_|_c$', ignore_case = T)) %>%
    str_replace_all('_', " ") %>%
    str_replace_all('-$', " ") %>%
...
cap_set = c('DNA', 'HR', 'HRD', 'MMR', 'CHEK2', 'CH', 'PD', 'II',
 ...
 '^ANC$', '^ALC$', '^AMC$', '^HGB$', '^MCV$', '^PLT$', '^WBC$', '^RDW$', 'XRT')
...
crosstab = function(D, x, y) {
    ...
    if (tolower(x) == 'total') {
        ...
        D %>% count(get(x)) %>% 
        rename('levels' = 'get(x)', 'total' = 'n') %>%
        mutate(variable = gsub('_', '-', x)) %>%
        select(variable, levels, total)
    } else if (is_continuous(D[[x]]) | class(D[[x]]) == 'integer') {
        # x is continuous
        D %>% 
        filter(!is.na(get(x))) %>%
        dcast(
...
tab_response = function(D, xs, y) {    
    lapply(
        xs,
        function(x) {crosstab(D, x, y)}
    ) %>% 
    Reduce(rbind, .)
}